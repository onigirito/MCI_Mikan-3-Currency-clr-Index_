...
現在の慣性が2025年末まで維持されると仮定し、以下のシナリオを設定する：
...
2025年末のPPP推定値を以下のように設定する（現在値および残り1ヶ月のインフレ率を考慮）：

- $\text{PPP}_{\text{USDJPY,2025末}} \approx 93.3$ （2025年11月: 93.2、ほぼ横ばい想定）
- $\text{PPP}_{\text{USDTRY,2025末}} \approx 21.2$ （2025年11月: 20.8、トルコインフレ継続）
...
$$
\begin{align}
\text{シナリオA}: \quad S_{\text{USDJPY}} &= 93.3 \times \exp(0.40 - (-0.11)) = 93.3 \times \exp(0.51) \approx 156 \
\text{シナリオB}: \quad S_{\text{USDJPY}} &= 93.3 \times \exp(0.38 - (-0.13)) = 93.3 \times \exp(0.51) \approx 156 \
...
月次データにおける典型的な変動幅（$\pm 0.05$程度）を考慮し、$m[\text{USD}] - m[\text{JPY}] \in [0.46, 0.56]$ の範囲を想定すると、USD/JPY の理論的変動範囲は以下となる：

$$
\begin{align}
\text{下限}: \quad S_{\text{USDJPY}} &= 93.3 \times \exp(0.46) \approx 148 \
\text{上限}: \quad S_{\text{USDJPY}} &= 93.3 \times \exp(0.56) \approx 158
\end{align}
$$

したがって、2025年末のUSD/JPY は月次変動の統計的範囲を考慮すると **148〜158円のレンジ内**で推移すると推定される。中心シナリオでは約155円となる。

### 限界と注意事項

本手法は以下の前提に基づいており、これらが崩れた場合には推定の妥当性を失う：

1. **慣性の維持**：現在のトレンドが継続することを前提とする。レジーム転換（第6章参照）が発生した場合、$m[i]$ の慣性は破綻する
2. **PPP推定の精度**：将来のPPPはインフレ率予測に依存するため、インフレ率の急変により推定精度が低下する
3. **外生的ショックの不在**：地政学的危機、金融政策の急転換等の外生的ショックは本モデルでは捉えられない
4. **バスケット内部の限定**：本推定は3通貨バスケット内の相対関係のみを反映し、バスケット外の要因（例：EUR、CNYの動向）は考慮されない
5. **短期予測の限界**：本手法は1〜3ヶ月程度の短期予測を想定しており、より長期の予測では不確実性が増大する

したがって、本手法は**短期的な構造分析の補助ツール**として位置づけられるべきであり、実際の取引判断には第7章で述べた外部指標（DXY、金利差、VIX等）との併用が不可欠である。
...
3. **実践的応用**：2025年末のUSD/JPY推定レンジは148〜158円（中心155円）
4. **短期予測ツールとしての位置づけ**：本手法は1〜3ヶ月程度の短期予測に適用可能だが、外部指標との併用を前提とし、レジーム転換時には妥当性を失う
