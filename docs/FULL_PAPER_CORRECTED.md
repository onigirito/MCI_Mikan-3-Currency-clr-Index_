...
- **データセット**: [`annual_mci_2005_2024.csv`](../dataset/annual_mci_2005_2024.csv)（2005-2024年の全データ）
- **仕様書**: データ算出方法の詳細仕様（[`specification_ppp_annual.md`](specification_ppp_annual.md)）
...
m[TRY]は-0.995（2022年1月）から-0.472（2025年11月）へ、約4年かけて段階的に上昇している。m[JPY]は+0.420（2022年1月）から-0.015（2025年11月）へ継続的に低下し、m[USD]は+0.48〜+0.62の範囲で推移、2025年11月時点で+0.487である。
...
ドル/リラ相場においてリラは一貫して下落を続けているが、MCIグラフではm[TRY]が0に向かって収束している。この一見矛盾した動きは、MCIが示すものの本質を理解することで説明できる。
...
月次データによれば、m[TRY]は2022年1月に-0.995で極めて深刻な割安状態からスタートし、複雑な経路を経て2023年7月に-0.766で底を打った。

- 2022年1月〜12月: -0.995 → -0.769（+0.226の改善）。エルドアン政権下での非正統的な金融政策が継続する中、m[TRY]は数値上は改善を示した。しかし2022年1月の-0.995という極端な割安状態自体が、2021年末の通貨危機（12月に史上最安値34リラ/ドルを記録）による異常値であった可能性が高い。2022年を通じた「改善」は、危機的水準からの一時的な回復を示すが、構造的な政策転換を伴わない不安定な状態が継続した。

- 2023年1月〜5月: -0.738 → -0.637（一時的な上昇傾向）。しかし構造的な政策転換には至らず、6月以降再び悪化。

- 2023年6月〜7月: -0.715 → -0.766（-0.052の悪化、底打ち）。2023年6月3日にシムシェク財務大臣が就任。就任直後、市場は政策転換の不確実性に直面し、m[TRY]は7月に-0.766で観測期間中の（2022年1月の異常値を除く）最低値を記録した。これは政策転換への過渡期における一時的なストレス極大化と解釈できる。

**構造転換と収束トレンドの形成（2023年8月〜）**

2023年8月以降、明確な収束トレンドが形成された。2023年8月〜12月は-0.744 → -0.711（上昇開始）、2024年通年は-0.696 → -0.517（大幅上昇）、2025年1月〜11月は-0.503 → -0.472（継続的上昇）である。
...
底打ち（2023年7月）からの約2年半で、m[TRY]は-0.766から-0.472へと約0.29ポイント上昇しており、年率約0.12ポイントの安定的な収束速度を示している。これは政策の一貫性と市場の信認回復を反映している。
...
月次MCIグラフにおいてm[TRY]は2023年後半から緩やかな上昇傾向を示している。これはPPPへの収束圧力（極端な割安状態は持続不可能であり、何らかの形で調整される）、構造的ストレスの解消（インフレ率の低下や金融政策の正常化が進行）、市場価格への反映遅延（構造的改善が価格に反映されるまでにはタイムラグがある）を反映している。

MCIの収束傾向は、将来的な通貨安定化の兆候と解釈できる可能性がある。ただしこれは「収束時期」を予測するものではなく、「収束方向への構造的圧力」を示すものである。
...
実勢為替レートは理論上 $0 \to \infty$ の範囲で変動しうるため、将来の変動域を直接的に推測することは困難である。一方、$m[i]$ 座標はゼロサム制約（$\sum m[i] = 0$）を満たすが、この制約自体は数学的に発散を妨げない。理論上は $m[\text{TRY}] = -100$、$m[\text{USD}] = +50$、$m[\text{JPY}] = +50$ のような極端な値も可能である。

しかし実証的には、$m[i]$ は限定的な範囲内で推移する傾向がある。その理由は、$m[i]$ の定義が為替レートのPPPからの乖離率に基づくためである。例えば $m[\text{TRY}] = -5$ となるには、実際の為替レート $S_{\text{USD/TRY}}$ がPPP値の約 $\exp(-5) \approx 0.007$ 倍、つまりPPPの1/140程度になる必要がある。このような極端な乖離は、デノミネーション実施やハイパーインフレーション期などを除き、現実的には極めて稀な現象である。

この実証的な限定性により、過去の観測範囲から将来の統計的可動域を推定することが可能となる。
...
*表注：観測幅は観測期間中に記録された最小値と最大値の差を示す。これは月々の変動幅（次表）とは異なり、期間全体での変動範囲を表す。*

この観測範囲は、年次データ（20年間）で観測された $\pm 0.8$ 程度の範囲と比較すると、USDは限定的だが、JPYとTRYはより広い変動を示している。月次補間データにおける月々の変動は以下のように穏やかである。

| 通貨 | 最大月次上昇 | 発生時期 | 最大月次下落 | 発生時期 | 平均月次変化 |
|-----|------------|---------|-------------|---------|-------------|
| USD | +0.058 | 2023年6月 | -0.032 | 2023年1月 | 0.010 |
| JPY | +0.047 | 2024年8月 | -0.061 | 2022年4月 | 0.019 |
| TRY | +0.055 | 2022年4月 | -0.077 | 2023年6月 | 0.020 |

注目すべき発生時期：

- **TRY最大下落（2023年6月、-0.077）**: シムシェク財務大臣就任月。政策転換期における構造的ストレスの極大化を反映。
- **TRY最大上昇（2022年4月、+0.055）**: 2022年前半の市場動揺期における一時的な調整局面。同月JPYは最大下落を記録しており、ゼロサム制約下での相対的な動きを示す。
- **JPY最大下落（2022年4月、-0.061）**: FOMCのQT加速合意とCPI 8.5%発表により米長期金利が急騰（2.4%→3.0%）。日米金利差の急拡大による円安圧力が生じた時期。
- **JPY最大上昇（2024年8月、+0.047）**: 日銀の金融政策転換期に対応。
- **USD最大上昇（2023年6月、+0.058）**: TRYの大幅下落に伴うバスケット内での相対的上昇（ゼロサム制約の結果）。
- **USD最大下落（2023年1月、-0.032）**: 2022年末から2023年初にかけての市場調整局面。
...
月次データから観察される傾向は以下である。USDは2023年6月の+0.622（ピーク）から緩やかに低下し、2025年11月時点で+0.487。JPYは2022年1月の+0.420から継続的に低下し、2025年11月時点でほぼ中立（-0.015）。TRYは2022年1月の-0.995から段階的に上昇、2023年7月に-0.766で一時的に底打ち後、2025年11月時点で-0.472まで回復している。
...
本章では月次データを用いたMCI分析により、年次データでは捉えきれない短期的な構造動態を明らかにした。

PPPの月次線形補間により分析に適したデータセットを構築することで、レジーム転換イベントやMCI座標トレンドが明確に可視化できた。観測期間中の主要イベントは、シムシェク財務大臣就任（2023年6月）、FOMCのQT加速（2022年4月）、日銀の金融政策転換（2024年8月）など、金融政策の転換点に集中している。

トルコリラの構造転換が詳細に捉えられた。m[TRY]は2023年7月に-0.766で底を打ち、その後約2年半で-0.472へと上昇した。これはシムシェク財務大臣のもとでの政策正常化により、リラの下落が「市場に抗う人工的な維持」から「インフレに見合った自然な調整」へと性質を変えたことを示している。価格は下がり続けているが、構造的ストレスは解消されている。

MCIの数理的特性として、可動域の限定性（Boundedness）が重要である。実勢為替レートは理論上無限の範囲で変動しうるが、m[i]座標はゼロサム制約により制約された空間内に写像される。この限定性により、統計的手法を用いた将来価格帯の推定が可能となる。

2025年12月の実践例では、3つのシナリオからUSD/JPYの中心シナリオ151〜156円、リスクレンジ147〜162円を導出した。リスクレンジは過去の最大級イベント（FOMCのQT加速、日銀政策転換）に匹敵する変動幅を想定しており、レジームチェンジ級の構造変化が発生した場合に到達しうる水準を示している。

本章で示した月次分析は、第7章の実務的運用指針をより短い時間軸で補完するものである。年次データによる長期的な構造把握と、月次データによる短期的な動態追跡を組み合わせることで、Mikan 3-Currency clr Index の実用性はさらに高まる。
