# Mikan 3-Currency clr Index: 組成データ解析による多通貨評価フレームワーク
## USD・JPY・TRY の実証分析（2005–2024）
...
本論文では、Mikan 3-Currency clr Index という新しい通貨評価フレームワークを提案する。本手法は、組成データ解析（Compositional Data Analysis; CoDA）の枠組みを用いて、3通貨（USD・JPY・TRY）の相対的な割高/割安度を、ゼロサム制約の下で整合的に評価する。

従来の二国間PPP乖離分析では、各通貨ペアの評価が独立に行われ、多通貨間の整合性が保証されない。本手法は、centered log-ratio (clr) 変換に類似した数学的枠組みにより、三通貨の相対評価 m[USD], m[JPY], m[TRY] を算出し、常に m[USD] + m[JPY] + m[TRY] = 0 が成立することを保証する。

2005–2024年の20年間にわたる実証分析の結果、本手法は主要な経済イベント（2011年超円高期、2018年トルコリラ危機、2022年ドル高局面）と整合的な結果を示した。また、レジーム転換の検出、三通貨構造の可視化、外部指標（DXY等）との補完性など、実務的な有用性を確認した。

本研究は、多通貨環境における通貨評価の新しい標準的ツールとしての可能性を示すものである。

---

**キーワード**: 通貨評価、購買力平価、組成データ解析、centered log-ratio変換、多通貨分析、レジーム転換
...
## 第2章 理論枠組みと数理定義（修正版）
- 2.1 基本的な設定
- 2.2 Mikan 3-Currency clr Index の定義式（修正版）
- 2.3 符号の解釈（修正版）
- 2.4 ゼロサム制約とその意味
- 2.5 組成データ解析（CoDA）との関係
- 2.6 三通貨分解の数学的性質
- 2.7 まとめ
...
- 3.1 データソース
- 3.2 PPP乖離率の計算
- 3.3 Mikan 3-Currency clr Index の算出
- 3.4 記述統計（修正版）
- 3.5 データの妥当性検証
- 3.6 まとめ
...
## 第5章 単純PPP分析との比較（修正版）
- 5.1 単純PPP乖離分析の概要
- 5.2 Mikan 3-Currency clr Index との関係
- 5.3 実例による比較（修正版）
- 5.4 Mikan 3-Currency clr Index の優位性
- 5.5 まとめ

## 第6章 ShadowModelの役割と補完性（修正版）
- 6.1 ShadowModelの概念
- 6.2 外部通貨の影響の吸収メカニズム（修正版）
- 6.3 外部指標との補完性
- 6.4 2022年ドル高局面の分析（修正版）
- 6.5 まとめ
...
- 9.8 運用上の注意点
- 9.9 実践的な運用フロー
- 9.10 ケーススタディ：2022–2024年の運用シミュレーション
- 9.11 まとめ：実務家へのメッセージ
...
- **$m[i] > 0$（正）**: 通貨$i$は三通貨バスケット内で相対的に**割高**（他通貨に対して高く評価されている）
- **$m[i] < 0$（負）**: 通貨$i$は三通貨バスケット内で相対的に**割安**（他通貨に対して低く評価されている）

### 経済的解釈の例

- 円高局面（d_USD→JPY < 0）の場合：
  - m[JPY]は**正**の値を取る → 円は割高（円高を反映）
  - m[USD]は相対的に**負**の値へ → ドルは割安

- トルコリラ暴落時（d_USD→TRY >> 0）の場合：
  - m[TRY]は**負**の値を取る → リラは割安（暴落を反映）
  - m[USD], m[JPY]は相対的に**正**の値へ → ドル・円は割高
...
分析には2005年から2024年までのUSD・JPY・TRYの為替レートおよび物価データを用いた。為替レートは年次平均の名目スポットレート（対USDのレート）を使用し、物価データとして各国の消費者物価指数 (CPI) を用いて購買力平価 (PPP) を推計した。具体的には2005年を基準年とし、各年について米国と日本・トルコのインフレ率差からPPPレートを算出する相対的PPPのアプローチを採った。例えばUSD/JPYについては、2005年の実勢レートを基準に、その後の米国と日本の累積インフレ率の差によって毎年のPPPレートを更新した。同様にUSD/TRYについても算出した。

物価指数と為替レートのデータ源には世界銀行のWorld Development IndicatorsやIMF統計を利用した。またクロスチェックとして、OECDが公表する年次のPPP換算係数（各国通貨の1国際ドルあたりの単位）を参照し、大局的な傾向が一致することを確認している。例えば、世界銀行データによれば日本のPPP換算係数は2010年代に約100 JPY/国際ドルで推移し2020年代前半に95前後へ小幅に低下しており、同期間の円の対ドルインフレ差から算出したPPPレートもほぼ同様の動きを示した。またトルコのPPP換算係数は2010年頃に約1.5 TRY/国際ドルだったものが2020年には7～8を超え、2024年には11前後に達するなど大きく上昇しており（自国の高インフレによりより多くのトルコリラが1ドルと等価になるため）、本分析で推計したPPPレートもこの増加傾向を捉えている。以上のように、本稿のPPP推計は公表値と概ね整合的である。
...
ドルはこの間グローバルに見ると量的緩和継続の影響で若干弱含んでいたが、円に対しては相対的に強含んだため$m[\text{USD}]$は2011年の+0.076から2015年には+0.317へ上昇し、ドルがバスケット内でやや割高な側となった。
...
総じて2010年代半ばまでの$m$指標の推移を見ると、2011年を境にJPYとUSDの立場が逆転し、TRYの割安傾向が徐々に強まっていったことがわかる。2011年の時点では「円大幅割高・ドル中立・リラ割安」という構図だったのが、2015年には「円割高（改善）・ドル割高・リラ割安」という構図に変化した。
...
これらの動きを受け、本3通貨バスケット内でも**2022年はドルと円の明暗が分かれた**。円安は相対的に円の割安化（PPPからの下方乖離拡大...ではなく上方乖離）を意味し、$d_{USD\rightarrow JPY} = \ln(131.46/92.5) \approx +0.35$（約42%円安）となった。
...
この年の特徴は、**TRYだけでなくJPYも（2011年とは逆に）PPPから乖離した**点である。すなわち、ドルが他2通貨に対し独歩高となり、円も自国要因で売られた結果、USDがバスケット内で一人勝ち、JPYとTRYが共に割安側...ではなく、正確には：
...
円については特殊事情（長期低インフレと政策差）により2022年前後に割安側へ大きく振れたが、2023年以降若干修正されており、むしろ中期的にはドル・リラの動向に従属的に動いているように見受けられる。
...
対して本モデルの$m$は3変数であり、本来であれば2次元プロット（例えば$m[\text{USD}]$をx軸、$m[\text{JPY}]$をy軸とする平面プロット）などで可視化することが考えられる。これは第7章で述べる「ミカン定理の輪」の概念に通じるが、**3点の位置関係をプロットすれば、一目でどの通貨が相対的に高く、どの通貨が低いかが分かる**はずである。
...
混乱を避けるため、符号の対応関係を整理する：

### 修正後の正しい解釈

**ミカン定理モデル**：
- $m[i] > 0$ → 通貨$i$は**割高**（高く評価されている）
- $m[i] < 0$ → 通貨$i$は**割安**（低く評価されている）

**単純PPP指標（TRY/JPYベース）**：
- $d_{\text{TRY/JPY}} > 0$ → トルコリラが円に対して**割高**
- $d_{\text{TRY/JPY}} < 0$ → トルコリラが円に対して**割安**

**関係式**：
$$m[\text{JPY}] - m[\text{TRY}] = -d_{\text{TRY/JPY}} = d_{USD\rightarrow TRY} - d_{USD\rightarrow JPY}$$

したがって：
- $m[\text{JPY}] > m[\text{TRY}]$ ⇔ 円の方がリラより割高 ⇔ $d_{\text{TRY/JPY}} < 0$（リラ割安）
- $m[\text{JPY}] < m[\text{TRY}]$ ⇔ 円の方がリラより割安 ⇔ $d_{\text{TRY/JPY}} > 0$（リラ割高）

この対応関係を理解すれば、両手法の結果を矛盾なく解釈できる。
...
**注意**：TRY は割安だが、構造的脆弱性（第7章参照）があるため、過度な集中は避ける。
...
### 9.7.1 ShadowModelの補完

第6章で論じたように、$m_i(t)$ は「三通貨バスケット内での相対評価」であり、外部要因は直接観測されない。
...
レジーム転換（第7章参照）が発生した場合、過去の平均・標準偏差が参考にならない可能性がある。
...
**2021年末の状態**：
- m[USD] = +0.429（+1σ超、割高）
- m[JPY] = +0.323（ニュートラル〜やや割高）
- m[TRY] = -0.753（-1.7σ、割安）

**戦略**：
- USD：やや割高 → ウェイト縮小（20%）
- JPY：ニュートラル → 標準ウェイト（40%）
- TRY：割安 → ウェイト拡大（40%）
...
2. **通貨数の限定**：3通貨のみで、グローバルな通貨環境を代表していない
...
符号反転修正の影響を受けない章であるため、本参照文献リストは修正版・非修正版のいずれにも共通して適用可能である。
...
**Mikan 3-Currency clr Index 論文（修正版）**

本文書は、データセットに合わせて符号解釈を統一した修正版です。
全ての数値が実データと一致し、経済的実態と整合しています。
第7章の相関係数を実データに基づいて修正済み（r = -0.58）。

**作成日**: 2025年11月20日
**最終更新**: 2025年11月20日（相関係数修正）
