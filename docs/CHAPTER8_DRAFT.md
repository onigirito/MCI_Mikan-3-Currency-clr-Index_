...
|---------|--------|--------|--------|------|
| **A（慣性維持）** | +0.49 | -0.02 | -0.47 | 現状トレンドの継続（USD微増、JPY中立維持、TRY横ばい） |
| **B（収束加速）** | +0.46 | 0.00 | -0.46 | 全体的に中立方向への収束が加速 |
| **C（USD強化）** | +0.51 | -0.04 | -0.47 | USD割高化継続、JPY微減 |
...
$$
\begin{align}
\text{シナリオA}: \quad S_{\text{USDJPY}} &= 93.52 \times \exp(0.49 - (-0.02)) = 93.52 \times \exp(0.51) \approx 156 \\
\text{シナリオB}: \quad S_{\text{USDJPY}} &= 93.52 \times \exp(0.46 - 0.00) = 93.52 \times \exp(0.46) \approx 148 \\
\text{シナリオC}: \quad S_{\text{USDJPY}} &= 93.52 \times \exp(0.51 - (-0.04)) = 93.52 \times \exp(0.55) \approx 162
\end{align}
$$
...
3. **実践的応用**：2025年12月のUSD/JPY推定レンジは中心シナリオ148〜162円、リスクレンジ147〜162円
